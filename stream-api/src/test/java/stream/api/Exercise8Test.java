...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
        ...
        Set<String> itemSetNotOnSale = customerStream.flatMap(customer -> customer.getWantToBuy().stream().map(Item::getName))
                .filter(customerItem -> !itemListOnSale.contains(customerItem)).collect(Collectors.toSet());
    ...
    public void havingEnoughMoney() {
        ...
        Predicate<Customer> havingEnoughMoney = (customer) -> customer.getBudget() > onSale.stream().map(Item::getPrice).count() ;
